...
// ============================================================
// SESSAO PERSISTENTE NO SUPABASE
// SQL: CREATE TABLE whatsapp_session (key TEXT PRIMARY KEY, value JSONB NOT NULL);
// ============================================================
async function useSupabaseAuthState() {
    const readData = async (key) => {
        try {
            const { data } = await supabase.from('whatsapp_session').select('value').eq('key', key).single();
            return data?.value || null;
        } catch { return null; }
    };
    const writeData = async (key, value) => {
        await supabase.from('whatsapp_session').upsert({ key, value }, { onConflict: 'key' });
    };
                ...
                set: async (data) => {
                    ...
                    for (const [type, ids] of Object.entries(data)) {
                        for (const [id, value] of Object.entries(ids)) {
                            if (value) tasks.push(writeData(type + '-' + id, value));
