...
def normalizar_cliente_rapido(v):
    ...
    # Correções principais de clientes da operação
    if "JDE" in s:
        return "JDE CAFÉ"
    if "SC" in s and ("DIST" in s or "DISTRIB" in s or "CAMACARI" in limpar_busca(s) or "CAMAÇARI" in s):
        return "SC DIST. CAMAÇARI"
    if "ATACADAO" in limpar_busca(s) and ("CT" in s or "SUL" in s):
        return "ATACADÃO CT SUL"
    if "DROGARIA" in s and ("SAO PAULO" in limpar_busca(s) or "SÃO PAULO" in s):
        return "DROGARIA SÃO PAULO"
    if "RAIA" in s or "DROGASIL" in s:
        if "SALVADOR" in s:
            return "RAIA DROGASIL SALVADOR"
        return "RAIA DROGASIL"
    if "ASSAI" in limpar_busca(s):
        return limpar_busca(s).replace("ASSAI", "ASSAÍ")
...
Exemplos:
M Jean D 3787760670 P 100 CL Atacadão CT Sul V 1021,05 L 08:51
M Jones D 3787780078 P 272 CL Drogaria São Paulo V 992,17 L 07:33 C 09:59 O L.F
M Luis D 3402132015 P 476 CL JDE CAFÉ V 1276,13 O CS OK C OK
